    ...
    def cts(self):
        while self._chatBackend.hasLines():
            ...
            if msg == "\001":
                ...
                print(f"not processed reply: {msg}")
...
class ProgrammedMotionController(QtCore.QObject):
    ...
    def _run(self):
        ...
        while len(self._buffer) and self._sentSteps - self._completedSteps < 2000:
            ...
            print
            "sent %d of %d; current N: %d; next command: %s" % (
            self._sentSteps, self._totalSteps, self._completedSteps, command)
            self._machine.cts()
